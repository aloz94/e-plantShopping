...
function ProductList({ onHomeClick }) {
  ...
  // Badge: total quantity in cart from Redux
  ...
  // Normalize plant before sending to Redux
  const normalizePlant = (plant) => ({
    ...plant,
    id: plant.id ?? plant.name, // prefer unique id
    cost:
      typeof plant.cost === 'string'
        ? Number(plant.cost.replace('$', ''))
        : plant.cost,
  });
  ...
  const handleAddToCart = (plant) => {
    ...
    setShowCart(true);
    setAddedToCart((prev) => ({ ...prev, [normalized.id]: true }));
  };
